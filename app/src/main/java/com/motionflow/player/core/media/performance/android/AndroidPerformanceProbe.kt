 ...
 *
 * Every reading here comes from public Android API, and each one is chosen for a unit the platform
 ...
 *
 * It holds a `PowerManager` rather than a `Context`, so nothing here can keep a UI context alive.
 *
 * ## Why the version checks read `SDK_INT` directly
 *
 * The thermal API arrived in API 29. The guard is written as an inline `Build.VERSION.SDK_INT` check at
 * each thermal call rather than against the injected [apiLevel], because that is the form the platform's
 * own analysis recognises: a field-based check compiles and runs, but reads to lint as an unguarded
 * call on a platform that may not have it. [apiLevel] stays for the *decisions* — what this platform can
 * measure, and what to record about it — which are pure and testable.
 */
class AndroidPerformanceProbe(
     ...
     * only when they happen, and a measurement cannot be contaminated by a timer asking the same question
    ...
    fun startThermalWatch(onStatus: (Int) -> Unit) {
        ...
        // playback thread by Media3's callbacks, and a thermal callback arriving on a binder thread would
        // be a data race on counters that no lock protects.
